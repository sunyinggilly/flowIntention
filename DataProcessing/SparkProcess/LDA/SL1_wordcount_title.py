...
if __name__ == "__main__":
    ...
    sql_fetch = "SELECT title FROM tic_dw.dwd_bp_job_post WHERE startdate like concat('2019-1', '%') or startdate like concat('2020', '%') or startdate like concat('2021', '%')"
    rdd = spark.sql(sql_fetch).rdd.filter(lambda x: x[0] is not None) #.filter(lambda x: random.random() < 0.1)
    rdd = rdd.flatMap(lambda x: jieba.cut(x[0])).map(lambda x:(x, 1))
    rdd = rdd.filter(lambda x: len(x[0]) >= 2 and not x[0].isdigit())
    rdd = rdd.reduceByKey(lambda a, b: a + b)  # word, count

    data = rdd.filter(lambda x: x[1] > 1000).collect()
    df = pd.DataFrame(data, columns=['word', 'count'])
    df.to_csv("out/word_count_title.csv", index=False)
    # df.to_csv("out/word_count_big.csv", index=False)
