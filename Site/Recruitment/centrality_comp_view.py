...
def set_centrality(cg, df_centrality, centrality, timebase, timecheck, mode='ratio', threshold=0):
    ...
    df_cent_base = df_centrality[df_centrality['time'] == '2021Q4']
    df_cent_base.rename(columns={centrality: '2021Q4'}, inplace=True)
    df_cent_change = pd.merge(df_cent_change, df_cent_base, on='name')

    df_cent_base = df_centrality[df_centrality['time'] == '2022Q4']
    df_cent_base.rename(columns={centrality: '2022Q4'}, inplace=True)
    df_cent_change = pd.merge(df_cent_change, df_cent_base, on='name')
    print(df_cent_change)
    sum_num = df_cent_change['timebase'].sum()
    if centrality != 'average_salary_diff':
        ...
        df_cent_change[centrality] = df_cent_change[['timebase', 'timecheck']].apply(lambda x: (x[1] - x[0]) / abs(x[0]) if abs(x[0]) > 100 else 0, axis=1)
    ...
    if mode != 'ratio' and ((timecheck == '2020Q1' and timebase == '2021Q1') or (timecheck == '2020Q2' and timebase == '2021Q2')):
        ...
        df_cent_change[centrality] = df_cent_change[['timebase', 'timecheck', '1pr']].apply(lambda x: x[1] / x[0] * x[2] - 1 if x[0] > threshold * sum_num  else 0, axis=1)

    ratio_max = 0.5
    if centrality in ['flow_in_net', 'flow_out_net', 'average_salary_diff']:
        df_cent_change[centrality] = df_cent_change[centrality].apply(lambda x: max(min(x,ratio_max),-ratio_max))
    # ----------------------------------------------- 设置点权 -----------------------------------------------------------------
...
def build_paints(cg, vertices, ctx, min_w=1e10, max_w=-1e10):
...
def build_ctx(request, ctx, mode='ratio'):
    ...
    set_centrality(cg, df_centrality, centrality=ctx['centrality'], timebase=ctx['timebase'], timecheck=ctx['timecheck'], 
                   mode=mode, threshold=0.0005)
    # print(ctx['centrality'])
    build_paints(cg, vertices, ctx)
    return render(request, 'centrality-comp-paint.html', ctx)


def init_page_paint_season(request):
    ctx = {'coarse': 'city', 'threshold': 0, 'timebase': '2019Q4', 'timecheck': '2020Q4', 'centrality': 'pr', 'graph_name': 'season'}
    return build_ctx(request, ctx, mode='ratio')


def init_page_paint_season_estimate(request):
    ctx = {'coarse': 'city', 'threshold': 0, 'timebase': '2019Q4', 'timecheck': '2020Q4', 'centrality': 'pr', 'graph_name': 'season'}
    return build_ctx(request, ctx, mode='estimate')
