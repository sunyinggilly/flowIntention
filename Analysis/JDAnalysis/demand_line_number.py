...
def solve(df, key_col):
    df_now = build_time_df(df, key_col, ['2019Q4', '2020Q1', '2020Q2', '2020Q3', '2020Q4', '2021Q1', '2021Q2'])
    ...
    # df_now['2020Q1'] = df_now[['2020Q1', '2019Q4']].apply(lambda x: (x[0] - x[1]) / x[1], axis=1)
    # df_now['2020Q2'] = df_now[['2020Q2', '2019Q4']].apply(lambda x: (x[0] - x[1]) / x[1], axis=1)
    # df_now['2020Q3'] = df_now[['2020Q3', '2019Q4']].apply(lambda x: (x[0] - x[1]) / x[1], axis=1)
    # df_now['2020Q4'] = df_now[['2020Q4', '2019Q4']].apply(lambda x: (x[0] - x[1]) / x[1], axis=1)
    # df_now['2021Q1'] = df_now[['2021Q1', '2019Q4']].apply(lambda x: (x[0] - x[1]) / x[1], axis=1)
    # df_now['2021Q2'] = df_now[['2021Q2', '2019Q4']].apply(lambda x: (x[0] - x[1]) / x[1], axis=1)

    df = pd.DataFrame()
    for key in ['2019Q4', '2020Q1', '2020Q2', '2020Q3', '2020Q4', '2021Q1', '2021Q2']:
    ...
    # df.to_csv("%s/%s_change_cities.csv" % (OUT_JD_PATH, key_col), index=False)
    df = df.reset_index(drop=True)
    df['Period'] = df['metric']
    df['Tier'] = df['level'].apply(lambda x: level_lst[x])
    df['Increase'] = df['increase']
    sns.lineplot(data=df, y='Increase', x='Period', hue='Tier')
    plt.savefig('%s/level_lineplot_%s.png' % (OUT_JD_PATH, key_col))
    plt.close()
...
if __name__ == "__main__":
    ...
    solve(df_centrality, 'jd_IT')
    solve(df_centrality, 'jd_Designer')
    solve(df_centrality, 'jd_PassengerTransport')
    solve(df_centrality, 'jd_FactoryWorker')
    solve(df_centrality, 'jd_Management')